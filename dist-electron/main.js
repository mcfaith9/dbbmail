...
function populate$2(processEnv, parsed, options2 = {}) {
...
let AxiosError$1 = class AxiosError extends Error {
  static from(error, code, config2, request, response, customProps) {
    ...
    const axiosError = new AxiosError(message, code || error.code, config2, request, response);
...
FormData$1.prototype.submit = function(params, cb) {
  ...
  if (options2.protocol === "https:") {
    request = https$1.request(options2);
  } else {
    request = http$1.request(options2);
  }
  ...
  getSession(authority, options2) {
    ...
    if (authoritySessions) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (!sessionHandle.destroyed && !sessionHandle.closed && require$$1$1.isDeepStrictEqual(sessionOptions, options2)) {
...
const estimateDataURLBytes = (url2, estimateBase64) => {
...
function getProxyEnvAgent(options2, configHttpAgent, configHttpsAgent) {
  return isHttps.test(options2.protocol) ? configHttpsAgent || https$2.globalAgent : configHttpAgent || http$2.globalAgent;
}
...
const httpAdapter = isHttpAdapterSupported && function httpAdapter2(config2) {
  return wrapAsync(async function dispatchHttpRequest(resolve$1, reject, onDone) {
    ...
    try {
      ...
      return reject(
        AxiosError$1.from(err, AxiosError$1.ERR_BAD_REQUEST, config2, null, null, {
          url: own2("url"),
          exists: true
        })
      );
...
function resolveConfig(config2) {
  ...
  if (auth) {
    ...
    try {
      ...
      throw AxiosError$1.from(e, AxiosError$1.ERR_BAD_OPTION_VALUE, config2);
    }
  }
